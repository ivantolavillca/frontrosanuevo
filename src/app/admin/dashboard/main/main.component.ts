...
import { EstadisticasService } from 'app/services/estadisticas.service'; // Importa el servicio
...
export class MainComponent implements OnInit {
  ...
  public maleCount: number = 0; // Propiedad para hombres
  ...
  ngOnInit() {
    this.chart3();
    this.CantidadDePersonasPorGenero();
  }
  CantidadDePersonasPorGenero() {
  
    this.estadisticasService.GetDatosGenero().subscribe(
      (data) => {
        this.maleCount = data['1'] || 0; // Asigna el valor para hombres
        this.femaleCount = data['2'] || 0; // Asigna el valor para mujeres
        this.isLoading = false; // Cambia la bandera cuando los datos se carguen
      },
      (error) => {
        console.error('Error al cargar las estadísticas:', error);
        this.isLoading = false; // Cambia la bandera incluso si hay un error
      }
    )
   
  }
  // CantidadDePersonasPorClasificacion(){
  //   this.estadisticasService.GetClasificacionPersona().subscribe(
  //     (data) => {
  //       this.performanceRateChartOptions.series[0].data = [
  //         data['1'] || 0, // Bajo
  //         data['2'] || 0, // Normal
  //         data['3'] || 0, // Sobrepeso
  //         data['4'] || 0, // Obesidad I
  //         data['5'] || 0, // Obesidad II
  //         data['6'] || 0, // Obesidad III
  //       ];
  ...
  private chart3() {
    this.performanceRateChartOptions = {
      ...
      xaxis: {
        categories: ['Bajo', 'Normal', 'Sobrepeso', 'Obesidad I', 'Obesidad II', 'Obesidad II'],
        title: {
          text: 'Categorías',
        },
      },
